...
- 一些难点说明：
  ...
  - chrome是如何判断是否有空闲时间，进而决定是否执行requestIdleCallback的？
    ...
    - 一个Vsync只会触发一次frame，如果一个frame提交给GPU以后，在等待下一次frame触发之前，
...
- 一个frame大致包括以下步骤：
  ...
  - HandleInputEvent：处理消息队列中的用户的交互事件，如点击、触碰、滚动等事件，这些交互事件优先级比较高，所以需要最优先处理
