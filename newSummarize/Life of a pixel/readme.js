     ...
     - 渲染进程
       - 主线程 Main Thread
         ...
         - 渲染引擎 (暴露一些桥接接口给js引擎来访问DOM和CSSOM)
           ...
           - 构建 Layer 树（图层数）
           - Paint绘制
             - 按照相应的顺序把layer信息生成带有一系列绘制操作命令的dislay item list；
             - 此阶段还负责把layers信息commit到合成器线程，这个操作是会block主线程的，
               等主线成接收到从合成线程返回的commit成功之后的消息之后就会继续开始主线程的工作了。
       - 合成器线程 Compositor thread（会阻塞主线程；可以直接接收input事件来处理，比如滚动合成层的滚动条，
         ...
         - Compositing合成（Combine layers to CCLayer, 当文档的各个部分以不同的layer绘制，
           ...
           - 在得到了layers信息以后，接下来的合成操作就不会block主线程了
